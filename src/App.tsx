...
function HomePage() {
  return (
    <main className="relative overflow-hidden">
      ...
      <Section id="roadmap" eyebrow="Roadmap" title="A phased rollout from brand launch to full app ecosystem" description="The roadmap shows clear progression from website and token presence to app architecture, wearable integrations, and broader health ecosystem expansion."><div className="relative grid gap-4 lg:grid-cols-5"><div className="pointer-events-none absolute left-0 right-0 top-7 hidden h-px bg-gradient-to-r from-cyan-400/0 via-cyan-300/40 to-cyan-400/0 lg:block" />{ROADMAP.map(({ phase, status, points }) => {   const isCurrent = status === "Current";    return (     <motion.div key={phase} {...fadeUp}>       <GlassCard         className={cn(           "relative h-full",           isCurrent && "border-cyan-400/30 bg-cyan-400/[0.07]"         )}       >         <div className="mb-4 flex items-center justify-between">           <div className="text-lg font-semibold text-white">{phase}</div>           <div             className={cn(               "rounded-full px-3 py-1 text-xs uppercase tracking-[0.2em]",               isCurrent                 ? "border border-cyan-400/20 bg-cyan-400/10 text-cyan-100"                 : "border border-white/10 bg-white/5 text-slate-300"             )}           >             {status}           </div>         </div>          <ul className="space-y-3 text-sm text-slate-300">           {points.map((point) => (             <li key={point} className="flex items-start gap-3">               <span className="mt-2 h-1.5 w-1.5 rounded-full bg-cyan-300" />               <span>{point}</span>             </li>           ))}         </ul>       </GlassCard>     </motion.div>   ); })}</div></Section>
